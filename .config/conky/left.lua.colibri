...
${execpi 60 today=`date +%_d`; cal | sed -n '3,8 p' | sed 's/^/${alignr} /' | sed s/"\(^\|[^0-9]\)$today"'\b'/'\1${color1}'"$today"'${color}'/}
  ...
  cpu    $color1 ${cpu cpu0} ${alignr} ${color} ${cpu cpu1}/${cpu cpu2}/${cpu cpu3}/${cpu cpu4}/${cpu cpu5}/${cpu cpu6}/${cpu cpu7}/${cpu cpu8}
  └ft    $color1 $freq ${color} ${alignr} ${hwmon 2 temp 1} / $color1${hwmon 4 temp 1}${color}
  ram    $color1 $memperc ${color} ${alignr} $mem
  avg    $color1 $loadavg ${color} ${alignr}
  prc    $color1 $running_processes ${color} / $processes ${alignr} $uptime
  dsk    $color1 $diskio ${color} ${alignr}
  └─a    $color1 ${diskio_write /dev/nvme0n1} ▼ $alignr ${color} ${diskio_read /dev/sda} ▲
